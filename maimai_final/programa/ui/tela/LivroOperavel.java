...
public class LivroOperavel {
    ...
    private void Tela() {
        ...
        statusLivro = func.criarLabel(430, 20, 100, 40, null, "", true, new Font("Arial", Font.BOLD, 15));
        statusLivro.setOpaque(true);
        statusLivro.setHorizontalAlignment(SwingConstants.CENTER);
        obj2.add(statusLivro);
    ...
    public void AtualizarTela(Livro livroVisto){
        operavel = func.encontrarNoOriginal(controle.biblio.getPrimeiro_livro(), livroVisto);;
        atualizarStatusVisual(operavel.getDisponibilidade());
        // Redimensiona a imagem para caber no espaço disponível
        ImageIcon imagemFinal = func.SetImageIcon(livroVisto.getPathFoto());
    ...
    private void BotoesNavegaveis(){
        ...
        botaoAlugar.addActionListener(a -> {
                boolean alocado = controle.biblio.retirarLivro(operavel);
                operavel.setDisponibilidade(false);
                if(alocado){
        ...
        botaoRemover.addActionListener(a -> {
           if(controle.biblio.removerLivro(operavel.getTitulo(), operavel.getAutor(), operavel.getAno(), operavel.getGenero())){
                func.removerFotoJason(operavel.getPathFoto(), controle, operavel);
        ...
        botaoDevolver.addActionListener(a -> {
                boolean sucesso = controle.biblio.devolverLivro(operavel);
                operavel.setDisponibilidade(sucesso);
                atualizarStatusVisual(sucesso);
            });
        ...
        voltar.addActionListener(ev -> {
            ...
            //
            controle.mem.atualizarDisponibilidadeNoJson(operavel);
        });

        obj.add(voltar);
    }

    public void atualizarStatusVisual(boolean disponivel) { // estado visual do livro
        if (disponivel) {
            statusLivro.setText("DISPONÍVEL");
            statusLivro.setBackground(Color.GREEN);
            statusLivro.setForeground(Color.BLACK);
        } else {
            statusLivro.setText("ALUGADO");
            statusLivro.setBackground(Color.RED);
            statusLivro.setForeground(Color.WHITE);
        }
    }
}
